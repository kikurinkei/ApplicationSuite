...
namespace ApplicationSuite.WindowModules.BoatRaceDataBank.Record.StaticFunctions.KTokens
{
    ...
    public static class KTokens
    {
        ...
        public static bool TryParse(string line, out KToken token)
        {
            ...
            // 3) 払戻金（単勝）行の判定（暫定: "ODDS" を含む行）
            if (line.Contains("ODDS", StringComparison.OrdinalIgnoreCase))
            {
                token = new KToken(KTokenKind.Odds);
                return true;
            }
