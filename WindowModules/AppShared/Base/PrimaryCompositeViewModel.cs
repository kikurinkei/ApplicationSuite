...
namespace ApplicationSuite.WindowModules.AppShared.Base
{
    ...
    /// </summary>
    public class PrimaryCompositeViewModel : ICompositeViewModel, INotifyPropertyChanged
    {
        public string? WindowUniqueId { get; set; } = null; // null許容型にした。

        private string? _CCVM { get; set; } = null; // null許容型にした。
        public string? CCVM
            {
            get => _CCVM;
            set
        ...
        private string? _pairedWindowUniqueId = null; // null許容型にした。
        public string? PairedWindowUniqueId
        {
            ...
            set
            {
                if (value != null)
                {
                    _pairedWindowUniqueId = value;
                }
            }
        }
        private bool? _isRealtimeSyncEnabled = null;
                ...
                if (_currentContentViewModel != value)
                {
                    ...
                    if (value is ISelectedAware aware)
                    {
                        ...
                        if (!string.IsNullOrEmpty(elementId))
                        {
                            ...
                            // CCVM は、PairSyncRelay経由で SecondaryCompositeViewModel の ManualView にも伝わる。
                            // CCVM プロパティに現在の elementId をセット

                            CCVM = elementId;

                            // リアルタイム同期が有効で、ペアウィンドウIDが設定されている場合、PairSyncRelayに通知
                            if (IsRealtimeSyncEnabled == true && !string.IsNullOrEmpty(PairedWindowUniqueId))
                            {
                                ApplicationSuite.Runtime.Pairing.PairSyncRelay.SetElementId(
                                    WindowUniqueId!, PairedWindowUniqueId, elementId);
                            }
